...
async def _run_ollama(api_key, model_id, message, base_url=None, history=None):
    ...
    while round_index <= MAX_TOOL_ROUNDS:
        ...
        # like "ps aux" instead of "running_apps". Dispatching that always fails, so when EVERY call
        # in the round is unresolvable it is corrected here without spending a tool round (the same
        ...
        invalid_calls = [
            ...
            if call["name"] == TOOL_NAME and args.get("command") not in PROCESS_COMMANDS
        ]
        ...
        if (
            ...
            for call, args in parsed_calls:
                ...
                correction = (
                    ...
                    f"like 'ps aux' cannot be passed directly. Pick exactly one id from this list: "
                    f"{', '.join(PROCESS_COMMANDS)}. If none of them answer the question, call "
...
async def run_process_agent(provider, api_key, model_id, message, base_url=None, history=None):
