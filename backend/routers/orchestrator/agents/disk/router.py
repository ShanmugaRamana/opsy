...
@router.websocket("/ws")
async def disk_agent_ws(websocket: WebSocket):
    """Runs the disk agent for exactly one request, streaming its events, then closes. Only ever
    ...
    try:
        ...
        try:
            request = DiskAgentRequest(**payload)
